...
def move_object(speed, move_distance, object_name):
    ...
    directions = [
        (move_distance, -move_distance),   # Move along +X-axis
        (move_distance, move_distance),   # Move along +Y-axis
        (-move_distance, move_distance),  # Move along -X-axis
        (-move_distance, -move_distance)   # Move along -Y-axis
    ]
...
if __name__ == '__main__':
    rospy.init_node('move_object_r', anonymous=True)
    try:
        # Randomize speed and define parameters
        MIN_SPEED = 0.2
        MAX_SPEED = 0.6
        MOVE_DISTANCE = 2.0
        OBJECT_NAME = "TriangleS"  # Replace with your Gazebo model name


        speed = random.uniform(MIN_SPEED, MAX_SPEED)  # Randomized speed
        rospy.loginfo(f"Randomized speed for TriangleS: {speed:.2f} m/s")
        #speed = 0.5
